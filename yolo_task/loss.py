...
class YoloLoss(nn.Module):
    ...
    def forward(self, y_p, y):
        ...
        box_predictions = exists_box * (best_box * y_p[..., 106:110] + (1 - best_box) * y_p[..., 101:105])
        box_targets = exists_box * y[..., 101:105]
        ...
        pred_box = (best_box * y_p[..., 105:106] + (1 - best_box) * y_p[..., 100:101])

        # (N*S*S, 1)
        object_loss = self.mse(torch.flatten(exists_box * pred_box), torch.flatten(exists_box * y[..., 100:101]))

        # FOR NO OBJECT LOSS

        # (N, S, S, 1) -> (N, S*S)
        no_object_loss = self.mse(torch.flatten((1 - exists_box) * y_p[..., 100:101], start_dim=1),
                                  torch.flatten((1 - exists_box) * y[..., 100:101], start_dim=1))

        no_object_loss += self.mse(torch.flatten((1 - exists_box) * y_p[..., 105:106], start_dim=1),
                                   torch.flatten((1 - exists_box) * y[..., 100:101], start_dim=1))

        # FOR CLASS LOSS

        # (N, S, S, 20) -> (N * S * S, 20)
        class_loss = self.mse(torch.flatten(exists_box * y_p[..., :100],  end_dim=-2),
                              torch.flatten(exists_box * y[..., :100], end_dim=-2))
